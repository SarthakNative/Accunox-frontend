import { Plus, MoreHorizontal, RefreshCw, Clock, ChevronDown, MoreVertical } from 'lucide-react';
...
const Header2 = () => {
  ...
  return (
    <div className="bg-[#f0f5fa] px-2 sm:px-6 py-3 sm:py-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto">
          ...
          {/* Refresh button */}
          <button className="p-1.5 sm:p-2 text-gray-600 hover:text-gray-900 transition-colors border rounded bg-white">
            <RefreshCw className="w-3 h-3 sm:w-4 sm:h-4" />
          </button>

          {/* More options button - Hidden on mobile, visible on sm and up */}
          <button className="hidden sm:block p-1.5 sm:p-2 text-gray-600 hover:text-gray-900 transition-colors  border rounded bg-white">
            <MoreVertical className="w-4 h-4" />
          </button>

          {/* Time period dropdown */}
          <div className="relative">
            <button className="flex items-center gap-1 sm:gap-2 px-2 py-1 sm:px-3 sm:py-2 bg-white border-2 border-indigo-900 rounded-md text-xs sm:text-sm font-medium transition-colors">
